...
4. **Git.** Stage completed work with `git add` as you go. Never run `git commit` or `git push` during the loop — the accumulated work is committed once, interactively, when the loop ends.
5. **Money.** Stop and report when an action involves payment, subscription, or an upgrade to a paid tier.
6. **Destructive.** Stop and report when an action deletes data, drops a database, removes files recursively, force-pushes, resets git history, or overwrites uncommitted work.
7. **Scope.** Skip anything leading outside the original task (unrelated tools, external signups, rabbit holes). Do only what the user asked.
...
- No `git commit` or `git push` appears anywhere in the actions taken; only `git add`.
