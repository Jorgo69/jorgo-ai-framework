...
2. **Trigger env.** When invoked from CI (`GITHUB_EVENT_NAME` set):
   ...
   - Comment body matches `@claude /implement` (or `/jai-dev:02-implement`) → `run`.
   - Comment body matches `@claude /review` → `review`.
3. **Repo state.**
   - Missing `.github/workflows/jai-async.yml` AND missing `.claude/jai-orchestrator.json` → `setup`.
...
**If repo state contradicts intent** (e.g. user says "run" but `.claude/jai-orchestrator.json` is absent), surface the conflict before delegating; never silently switch.
...
| 03  | `generate-workflow`               | Emit `.github/workflows/jai-async.yml` from `assets/setup/workflow-template.yml`             |
| 04  | `generate-local-script`           | Emit poll/daemon scripts (local mode only) from `assets/setup/local-*-template.sh`            |
| 05  | `write-config`                    | Persist `.claude/jai-orchestrator.json` from `assets/setup/config-template.json`             |
| 06  | `bootstrap-labels`                | Create `to-implement` / `to-review` / `claude/*` labels via `gh`                              |
| 07  | `install-user-scope-plugins`      | Install `jai-orchestrator` + `jai-dev` at user scope (local mode)                           |
...
```shell
echo "aidd:step-end jai-orchestrator:00-async-dev"
```
...
- `assets/setup/workflow-template.yml` - `.github/workflows/jai-async.yml` skeleton.
- `assets/setup/local-poll-template.sh` - local-mode poll script.
- `assets/setup/local-daemon-template.sh` - local-mode daemon script.
- `assets/setup/config-template.json` - `.claude/jai-orchestrator.json` skeleton.
