...
## Output
A file at `.github/workflows/jai-async.yml`.
...
3. Substitute placeholders:
   ...
   - `__CLAUDE_AUTH_LINE__` -> matches `answers.claude_action_auth.mode` (the secret name is resolved per-run via the dispatch step `route_account`, which reads `.claude/jai-orchestrator.json`'s `account_routing` and `default_secret_name`):
...
4. If `.github/workflows/jai-async.yml` already exists, prompt the user to overwrite or skip.
