...
## Output
A file at `.claude/jai-orchestrator.json`.
...
4. If `.claude/jai-orchestrator.json` already exists, diff against the new config and ask the user to confirm overwrite.
...
After running, `jq '.version, .mode, .labels.to_implement' .claude/jai-orchestrator.json` returns the plugin version, the chosen mode, and the `to-implement` label name. `jq -e '.max_iterations >= 1' .claude/jai-orchestrator.json` exits 0.
