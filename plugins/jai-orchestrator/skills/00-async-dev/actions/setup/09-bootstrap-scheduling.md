...
Schedules `scripts/jai-async-poll.sh` via the cheapest path that fits the user's needs. **Never recommends OS-level cron**, but for the local daemon path uses tmux/launchd/systemd so the user keeps Claude Code's Tasks quota for other things.
...
```json
{
  ...
  "supervisor": "tmux",
  "supervisor_artefact": "scripts/jai-async-daemon.sh"
}
```
...
4. Branch on the choice:
   - **C (local daemon)**:
     - Render `scripts/jai-async-daemon.sh` from `assets/local-daemon-template.sh` (no placeholders to substitute today; the daemon delegates to `jai-async-poll.sh`). Write with mode `0755`.
   ...
   - **B (manual)**: print `./scripts/jai-async-poll.sh` and a reminder that nothing runs until invoked. Set `path = "manual"`.
   ...
   - **E (cloud routine)**: invoke the runtime `/schedule` skill via the `Skill` tool with a cron expression (must be at least hourly) and the prompt `Use skill jai-orchestrator:00-async-dev with action=run on the next ready issue in <owner>/<repo>`. Capture the routine id. Set `path = "schedule_routine"`.
5. Emit the structured result.

## Test

**Path C (default)**: action writes `scripts/jai-async-daemon.sh` with mode 0755, prints the chosen supervisor's one-liner with the absolute repo path filled in, and returns `path = "local_daemon"` with the chosen interval. The daemon script does NOT mention Claude Code Scheduled Tasks.

**Path B**: returns `path = "manual"` and prints exactly `./scripts/jai-async-poll.sh`.
