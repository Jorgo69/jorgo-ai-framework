...
Full contract for picking a sub-flow inside `jai-orchestrator:00-async-dev`. The router walks these signals in order; the first match wins.
...
```yaml
prompt: "Use skill jai-orchestrator:00-async-dev with action=run on issue #..."
prompt: "Use skill jai-orchestrator:00-async-dev with action=review on PR #..."
```
...
- `run`: `@claude\s+/(implement|jai-dev:02-implement|run)`
- `review`: `@claude\s+/(review|jai-dev:05-review)`
...
| -------------------------------------------------------------------------------- | -------------------------------------------- |
| `.github/workflows/jai-async.yml` missing AND `.claude/jai-orchestrator.json` missing | `setup`                                |
| `.claude/jai-orchestrator.json` missing but workflow present                    | `setup` (incomplete install)                 |
...
- **User says "run" but config is absent.**
  Surface the conflict and stop: `Setup must complete before run. Run /jai-orchestrator:00-async-dev with action=setup first.`
