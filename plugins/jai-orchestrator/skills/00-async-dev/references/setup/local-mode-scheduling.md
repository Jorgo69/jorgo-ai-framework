...
Five paths to run `scripts/jai-async-poll.sh` periodically. **Remote (GitHub Actions) is the recommended primary path**; the local paths below are for cases where Actions is unavailable or undesirable.
...
Set `mode = "remote"` (or `"both"`) at setup. The skill writes `.github/workflows/jai-async.yml` and stores the auth secret on the repo. GitHub triggers the workflow on the configured label or mention; nothing runs locally.

## Path B -- Local manual

```bash
./scripts/jai-async-poll.sh --dry-run    # preview what would run
./scripts/jai-async-poll.sh              # run once, now
```
...
A long-running shell loop. **Not** a Claude Code scheduled task: the loop is just a `while true; do ./scripts/jai-async-poll.sh; sleep N; done`. Each tick that has nothing to do exits immediately and consumes no Anthropic token; ticks that find a trigger fire a single `claude -p`.

Setup writes `scripts/jai-async-daemon.sh`. Wrap it with a supervisor of your choice:

**tmux** (simplest, manual restart on reboot):
```bash
tmux new -d -s jai-async './scripts/jai-async-daemon.sh 300'
tmux ls           # see it
tmux attach -t jai-async   # watch logs
tmux kill-session -t jai-async
```
...
<plist version="1.0">
  <dict>
    ...
    <array>
      ...
      <string>/abs/path/to/repo/scripts/jai-async-daemon.sh</string>
      <string>300</string>
    </array>
    <key>RunAtLoad</key><true/>
    <key>KeepAlive</key><true/>
    <key>StandardOutPath</key><string>/tmp/jai-async-daemon.log</string>
    <key>StandardErrorPath</key><string>/tmp/jai-async-daemon.err</string>
  </dict>
</plist>
```
...
```ini
[Unit]
Description=jai-orchestrator async daemon
[Service]
WorkingDirectory=/abs/path/to/repo
ExecStart=/bin/bash /abs/path/to/repo/scripts/jai-async-daemon.sh 300
Restart=always
[Install]
WantedBy=default.target
```
Save as `~/.config/systemd/user/jai-async-daemon.service`, then `systemctl --user enable --now jai-async-daemon`.
...
4. Prompt: `Run ./scripts/jai-async-poll.sh and report what was processed.`
...
3. Prompt: `Use skill jai-orchestrator:00-async-dev with action=run on the next ready issue in {owner}/{repo}.`

## Sanity check before scheduling anything

```bash
./scripts/jai-async-poll.sh --dry-run
./scripts/jai-async-poll.sh
```
