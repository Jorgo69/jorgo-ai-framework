...
Operate autonomously from the request to a draft pull request, deciding freely within each zone. Two points always require explicit user approval before proceeding: the plan produced by Frame, before Deliver starts implementing it; and the commit/push produced by Deliver, before its diff and message are applied. Also ask before spending money, taking any other irreversible action, or making a decision that requires user authority. Read only the current zone reference. Verify that every named provider is installed before calling it.
...
```shell
echo "aidd:step-end jai-orchestrator:01-sdlc"
```
