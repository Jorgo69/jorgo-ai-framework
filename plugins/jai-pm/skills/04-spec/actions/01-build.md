...
6. **Declare.** When the request names a backlog item — a ticket already resolved (e.g. by `/jai-pm:01-ticket-info`), or a path to its Markdown artefact — and the folder carries no `backlog-link.json` yet, write one there, naming that item on whatever support it lives:
