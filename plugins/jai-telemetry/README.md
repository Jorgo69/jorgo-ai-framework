...
tells you that `jai-dev:02-implement` spent 78,188 of them, on task `2026_09_01_the-upward-link`,
...
period    2026-08-21 to 2026-08-21
  ...
  by step    of tokens
    jai-ui:01-hello           67%   78,188 tokens    stated by the tool
    jai-ui:01-hello           33%   38,490 tokens    from a journal interval
```
...
aidd plugin install jai-telemetry
```
...
A task folder can say which backlog item it delivers. `jai-pm:04-spec` and
`jai-dev:01-plan` write `backlog-link.json` beside `spec.md` and `plan.md` when the request
